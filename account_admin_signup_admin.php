...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($Fullname) || empty($Phone_Number) || empty($Password) || empty($Email)) {
        ...
        echo "<script>window.location='http://localhost/warehouse/a_signup'</script>";   
        exit;
    }
    ...
    $find = "SELECT MAX(CAST(SUBSTRING(AdminID, 2) AS UNSIGNED)) AS Latest FROM Admin";
    ...
    $AdminID = 'A' . str_pad($latest + 1, 2, '0', STR_PAD_LEFT);

    
    
    
    $exist = "SELECT * FROM admin WHERE Email=?";
    if ($stmt = $sambungan->prepare($exist)) {
        $stmt->bind_param('s', $_POST['Email']);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows > 0) {
            echo "<script>alert('Email already exit ! Please use other email or reset password.')</script>";
            echo "<script>window.location='http://localhost/warehouse/a_signup'</script>";
            exit;
        } else {
            $insert = "INSERT INTO admin (AdminID, Fullname, Phone_Number, Date, Password, Email) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($sambungan, $insert);
            mysqli_stmt_bind_param($stmt, "ssssss", $AdminID, $Fullname,  $Phone_Number, $Date, $Password, $Email); // corrected parameter name

            if (mysqli_stmt_execute($stmt)) {
                echo "<script>alert('Sign up successful! The AdminID had sended to the admin email.')</script>";
            } else {
        ...
        $mail->Subject = 'AdminID';
        $mail->Body = 'Thanks for register account from our website. This is the ID for you to login : ' . $AdminID;
...
<body >
 ...
 <div class="form-container">
      <p class="title">Register Admin Account</p>
