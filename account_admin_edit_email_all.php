...
if($Previous == 'admin'){
    $update = "UPDATE admin SET Email=? WHERE AdminID='$use'";
...
elseif($Previous == 'user'){
    $update = "UPDATE users SET Email=? WHERE UserID='$use'";
    $binding = $sambungan->prepare($update);    
    $binding ->bind_param('s',$New);
    $role ='users';
}
...
elseif($Previous == ' manager'){
    $update = "UPDATE manager SET Email=? WHERE ManagerID='$use'";
    $binding = $sambungan->prepare($update);    
    $binding ->bind_param('s',$New);
    $role ='manager';
...
<form method="post" >
    ...
    <h1>Edit Admin Email</h1>
    <?php } elseif($Previous == 'manager'){ ?>
    <h1>Edit Manager Email</h1>  
     <?php } ?>
    <input placeholder="Enter User ID" class="input-field" type="text" name="userID">  
    <input placeholder="Enter New Email" class="input-field" type="text" name="New">
